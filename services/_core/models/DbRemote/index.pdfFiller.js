...
module.exports = class ApiV5 extends CoreRequest {
  ...
  editorAuth(
    ...
    const wrappedCallback = this.logRequest('editorAuth', context, false, callback);
  ...
  editorDefaults(
    ...
    const wrappedCallback = this.logRequest('editorDefaults', context, false, callback);
   ...
   */
  getDocumentContent(viewerId = false, projectId = false,
    version = false, allowExtraData = [], callback = () => { }) {
    ...
    this.send(options, wrappedCallback);
  ...
  saveMappingDocument(data = {}, callback) {
    const {
    ...
    const wrappedCallback = this.logRequest('saveMappingDocument', context, true, callback);

    const options = {
      url: `${this.url}${this.endpoints.mappingFields}`,
      method: 'POST',
      json: true,
      headers: {
        userId: viewerId,
    ...
    return this.send(options, (err, result) => {
      if (err) {
        err = this.errorFactory.apiError(err, null, 'saveMappingDocument');
  ...
  getMappingDocument(data = {}, callback) {
    const {
      device, os, sessionHash, projectId, viewerId, clientType, integration, demo,
    } = data;
    ...
    const wrappedCallback = this.logRequest('getMappingDocument', context, false, callback);

    const options = {
      url: 'https://www.pdffiller.com/flash/mapping_demo.json',
      method: 'GET',
      json: true,
      bodyAsJson: true,
    };
    ...
    return this.send(options, (err, result) => {
      if (err) {
        err = this.errorFactory.apiError(err, null, 'getMappingDocument');
  ...
  loadTempPdf(data, callback) {
    ...
    const wrappedCallback = this.logRequest('loadTempPdf', context, true, (err, body) => {
      ...
      return callback(body ? null : 'shortUrl missing', body);
    });
  ...
  editorDestroy(
    ...
    this.sendRequest(options, (err, result) => {
      if (err) {
        err = this.errorFactory.apiError(err, null, 'editorDestroy');
      }

      return wrappedCallback(err, result);
    });
   ...
   */
  // getEditorSettings(
  // viewerId = false, projectId = false, projectVersion=false, clientType=false, launch=false,
  // modeId=false, access=false, partKey=false, callback
  // ) {
  //
  //   if (!viewerId) throw new Error('getEditorSettings(), invalid viewerId passed.');
  //   if (!projectId) throw new Error('getEditorSettings(), invalid projectId passed.');
  //   // if (!projectVersion) { TODO FUCK 0 is fucking false
  //    throw new Error('getEditorSettings(), invalid projectVersion passed.');
  //    }
  //   if (!launch) throw new Error('getEditorSettings(), invalid launch passed.');
  //   if (!clientType) throw new Error('getEditorSettings(), invalid clientType passed.');
  //   if (!clientType) throw new Error('getEditorSettings(), invalid clientType passed.');
  //   if (!modeId) throw new Error('getEditorSettings(), invalid modeId passed.');
  //   if (!access) throw new Error('getEditorSettings(), invalid access passed.');
  //   if (!partKey) throw new Error('getEditorSettings(), invalid partKey passed.');
  //
  //   let options = {
   ...
   * @param callback
   */
  addImages(userId, attributes, callback) {
    const options = {
  ...
  deleteImages(userId, systemId, callback) {
    const options = {
  ...
  listSignatures(userId, callback) {
    const options = {
  ...
  addSignatures(userId, attributes, callback) {
    const data = attributes.postData;

    delete attributes.postData;
  ...
  deleteSignatures(userId, systemId, callback) {
    const options = {
